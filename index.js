...
app.post("/addcomment", async (req, res) => {
    ...
    await connection.query(
        "INSERT INTO BBY_26_groupcomments (groupID, comment) values (?, ?)", [req.body.groupID, req.body.comment],
        function (error, results, fields) {
            if (error) {
                connection.end();
            } else {
...
app.post("/getcomment", async (req, res) => {
    ...
    connection.connect();
    const [rows, fields] = await connection.execute("SELECT * FROM bby_26_groupcomments WHERE groupID = " + req.body.groupID);
    for(let i = 0; i < rows.length; i++){
        commarr.push(rows[i].comment)
    }

    await connection.end();

    res.send(JSON.stringify(commarr));
